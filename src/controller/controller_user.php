...
if (isset($_POST['ID_user'])) {
    ...
    // Thực hiện hành động xóa hoặc bất kỳ logic nào bạn cần
    $deleteResult = $user->delete($ID);

    // Hiển thị thông báo dựa trên kết quả của hoạt động xóa
    if ($deleteResult) {
        echo "<script> alert('Người dùng đã bị xóa thành công.'); window.location.href = 'your_redirect_page.php';</script>";
    } else {
    ...
    if($success == 1){
      echo '<script>alert("Đăng ký thành công.");</script>';
        ...
        // Gọi hàm update_user
        $updateResult = $user->update_user($ID, $username, $user_email, $old_pass, $new_pass);
    if ($updateResult) {
